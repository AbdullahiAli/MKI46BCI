...
def main():
    ...
    pygame.display.set_caption('Hybrid Calibration Stimulus')
    ...
    while True: # display update loop
        ...
        if run:
            ...
            if screen_state.pause_time > 60000:
                if target == 'left':
                    target = 'right'
                else:
                    target = 'left'
...
def process_event(left_rect, right_rect, pos, target):
    """
    Method that processes events and sends the appriopate events to the 
    buffer. If the subject is attending to the rectangle send a "target" event
    otherwise send a "non-target" event.
    
    Params:
        left_rect: left rectangle
        right_rect: right rectangle
        pos: the position of the rectangle to process (0: left, 1: right)
        target: the rectangle the subject is currently attending to (left, right)
    """
    if pos == 0:
        if target == 'left':

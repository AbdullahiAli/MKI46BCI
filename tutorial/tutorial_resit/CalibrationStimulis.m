...
sendEvent('stimulus.cue', cue);
set(h,'string',cue,'color', 'gr', 'visible', 'on'); % display the cue with a green color
drawnow;
sendEvent('stimulus.trial','startTrial'); % send event annotating the start of current trial
sleepSec(cueDuration); % sleep for cue duration
...
set(h,'string',char, 'color', [1 1 1], 'visible', 'on'); % update the text displayed
sendEvent('stimulus.letter', char);
if char == cue
    sendEvent('stimulus.letter','target'); 
else
    sendEvent('stimulus.letter','non-target'); 
end
